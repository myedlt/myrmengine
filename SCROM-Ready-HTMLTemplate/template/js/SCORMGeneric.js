...
function doNothing() {return true;} //for backward compatible error catching in case people use the old index page
	...
	//alert('self.data.PageArray.length' +self.data.znThisPage + ' ' +self.data.PageArray.length);
	 if (!APIOK()){
		...
		return false;		
			...
			SCOFinish(); 
			//alert ('gbfinishdone was true but not at end');
		 }
}
		 ...
		 if ( (g_bShowApiErrors)) { //if these scripts have been loaded (prevents js errors if an old index page is being used)
			SetCookie('referenceMode',false,1);
				if (ReadCookie('referenceMode')==false){
			        alert('It appears you did not launch this from the LMS. No score will not be recorded. Module is in "no-credit mode. "');
				    SetCookie('referenceMode',true,1);
				}
		
		window.status="It appears you did not launch this from the LMS. No score will not be recorded";
		}//if showApiErrors
}//function
