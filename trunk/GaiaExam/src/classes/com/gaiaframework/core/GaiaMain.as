...
package com.gaiaframework.core
{
	...
	public class GaiaMain extends Sprite
	{
		...
		protected var centerCount:int = 0;
		...
		public function GaiaMain()
		{
			...
			model = new SiteModel(stage.loaderInfo.parameters.siteXML || siteXML);
		...
		protected function centerSite(w:int, h:int):void
		{
			...
			addEventListener(Event.ENTER_FRAME, centerEnterFrame, false, 0, true);
		}
		protected function centerEnterFrame(event:Event):void
		{
			onResize(new Event(Event.RESIZE));
			if (centerCount++ > 2)
			{
				centerCount = 0;
				removeEventListener(Event.ENTER_FRAME, centerEnterFrame);
			}
		}
