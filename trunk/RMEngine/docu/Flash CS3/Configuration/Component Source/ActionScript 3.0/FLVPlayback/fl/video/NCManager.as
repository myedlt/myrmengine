...
package fl.video {
	 ...
	 */
		public function connectToURL(url:String):Boolean {
			...
			return (canReuse || connectRTMP());
				...
				if ( parseResults.streamName.indexOf("?") < 0 &&
	 ...
	 *
		...
		flvplayback_internal function nextConnect(e:TimerEvent=null):void {
			...
			var xnURL:String = protocol + ((_serverName == null) ? "" : "/" + _serverName + ":" + port + "/") + ((_wrappedURL == null) ? "" : _wrappedURL + "/") + _appName;
			//ifdef DEBUG
		...
		flvplayback_internal function cleanConns():void {
			...
			if (_tryNC != null) {
				for (var i:uint = 0; i < _tryNC.length; i++) {
					if (_tryNC[i] != null && _tryNC[i] != undefined) {
						//ifdef DEBUG
		...
		flvplayback_internal function connectOnStatus(e:NetStatusEvent):void {
			...
			if (e.info.code == "NetConnection.Connect.Success") {
				//ifdef DEBUG
				//debugTrace( "Connection " + RTMP_CONN[e.target.client.connIndex].protocol +
				//			":" + RTMP_CONN[e.target.client.connIndex].port + " succeeded!" );
				//endif
				...
				cleanConns();
						...
						( e.target.client.connIndex == (RTMP_CONN.length - 1) ) ) {
				...
				//ifdef DEBUG
				//debugTrace( RTMP_CONN[e.target.client.connIndex].protocol + ":" +
				//			RTMP_CONN[e.target.client.connIndex].port +
				//			" onStatus:" + e.info.code);
				//endif
			}
		}
