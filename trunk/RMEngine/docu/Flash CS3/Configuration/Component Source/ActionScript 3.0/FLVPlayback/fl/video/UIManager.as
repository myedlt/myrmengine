	...
	public class UIManager {
			...
			if (borderPrevRect == null || !borderPrevRect.equals(newBorderRect)) {
				...
				if (borderScale9Rects == null) {
					...
					for (var i:int = 0; i < borderScale9Rects.length; i++) {
						...
						drawMatrix.a = 1;
						if (i == 1 || i == 4 || i == 7) {
							var middleScaleX:Number = (newBorderRect.width - (nextX * 2)) / maskRect.width;
						...
						drawMatrix.d = 1;
						if (i >= 3 && i <= 5) {
							var middleScaleY:Number = (newBorderRect.height - (nextY * 2)) / maskRect.height;
		...
		flvplayback_internal function setSkin(index:int, avatar:DisplayObject):void {
			...
			if (index >= NUM_CONTROLS) {
				...
				ctrl = setupButtonSkin(index);
				skin_mc.addChild(ctrl);
				ctrlData = ctrlDataDict[ctrl];
			} else {
				switch (index) {
				case PLAY_PAUSE_BUTTON:
					ctrl = setTwoButtonHolderSkin(index, PLAY_BUTTON, "play_mc", PAUSE_BUTTON, "pause_mc");
					ctrlData = ctrlDataDict[ctrl];
					break;
				case FULL_SCREEN_BUTTON:
					ctrl = setTwoButtonHolderSkin(index, FULL_SCREEN_ON_BUTTON, "on_mc", FULL_SCREEN_OFF_BUTTON, "off_mc");
					ctrlData = ctrlDataDict[ctrl];
					break;
				case MUTE_BUTTON:
					ctrl = setTwoButtonHolderSkin(index, MUTE_ON_BUTTON, "on_mc", MUTE_OFF_BUTTON, "off_mc");
					ctrlData = ctrlDataDict[ctrl];
					break;
				case SEEK_BAR:
				case VOLUME_BAR:
		...
		flvplayback_internal function setTwoButtonHolderSkin( holderIndex:int,
		                                                      ...
		                                                      secondIndex:int, secondName:String ):Sprite {
			var subBtn:Sprite;
			var ctrl:Sprite;
			var ctrlData:ControlData;
			...
			subBtn = setupButtonSkin(secondIndex);
		...
		flvplayback_internal function setupButtonSkin(index:int):Sprite {
			//ifdef DEBUG
		...
		flvplayback_internal function enterFullScreenTakeOver():void {
			if (!_fullScreen || cacheStageAlign != null) return;
		...
		flvplayback_internal function exitFullScreenTakeOver():void {
			if (cacheStageAlign == null) return;
